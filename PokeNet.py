...
class UNET(): #Main class. All methodes deemed usefull to achief more robust image segmentation
    ...
    def patterns(self,c_size=5120,boundry=256,images=1000,draw=True,bg_noise=True):#pattern generator, Produces random patterns of variable size comprized of triangles squares circles and stars.
        ...
        if draw==True:#Just draws the whole canvas for fun
            import matplotlib.pyplot as plt
            plt.axis('off')
            plt.imshow(canvas,vmin=0,vmax=1)
            self.save_img(plt,'patterns')
            
        if bg_noise==True: # add semi random noise to the images
            from PIL.ImageGrab import grab
            background=np.asarray(grab())/256
            bgs=background.shape
            bg=background.reshape(bgs[0]*bgs[1],bgs[2])
            for x in range(images):
                data_slice=data[x]
                posx,posy=np.where(np.sum(data_slice,axis=-1)==0)
                data_slice[posx,posy,:]=bg[np.random.randint(bg.shape[0],size=len(posy))]
                hh=x+1
                how_far=hh/images
    ...
    def __jaccard_distance(self,y_true, y_pred, smooth=100):
    ...
    def dice_cof_iou(self,a=None,b=None,size=3,draw=True,threshold=10**-1):
        if type(a)==type(None) or type(b)==type(None):
            a=self.test_data
            b=self.test_mask
        pred=np.where(self.predict(a,b,size=size,draw=draw)<threshold,0,1)
        tp=np.sum((pred+b)>1)
        fp=np.sum((b-pred)<0)
        tn=np.sum((pred+b)==0)
        fn=np.sum((b-pred)==1)
        print('Dice Coeffizient:', end=' ')
        print((2*tp)/(2*tp+fp+fn))
        print('Jaccard score:', end=' ')
        print(tp/(tp+fp+fn))
    
    
    __def dice_cof(self,a=None,b=None,size=3,draw=True,threshold=10**-1):
        if type(a)==type(None) or type(b)==type(None):
            a=self.test_data
            b=self.test_mask
        pred=np.where(self.predict(a,b,size=size,draw=draw)<threshold,0,1)
    ...
    __def iou(self,a=None,b=None,size=3,draw=True,threshold=10**-1):
        if type(a)==type(None) or type(b)==type(None):
            a=self.test_data
            b=self.test_mask
        pred=np.where(self.predict(a,b,size=size,draw=draw)<threshold,0,1)
    ...
    def selectiv_(self,data,mask,epsilon=.01,arg_min=True,against_mask=False): # main algorithm for generating augmented training data generates all possible combination of postive negativ or neutral pertubation choosing the one with the highest accuracy score to add to trainings set. The mask variable is pretty much useless and stems from legacy
        string_print='[ ] 0.00%'
        print(string_print,end='\r')
        targed_class_mask=[]
        if len(data.shape)==3:
        ...
        for xy in range(self.depth): #creates depending on the depth of the mask different full area classes to apply FGSM on 
                ...
                if line==pos-1:
                    ...
                    for i in range(pos):
                        if dim[i]!=iters-1:
                            ...
                            dim[i]=0
            if arg_min==True:# choice of highest accuracy or lowest accuracy depending on constructor definition argmin is the lowest difference between ground truth and prediction
